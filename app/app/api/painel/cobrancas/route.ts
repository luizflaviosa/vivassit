...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select(
      'id, external_reference, asaas_payment_id, patient_name, patient_email, patient_phone, doctor_name, consultation_date, consultation_value, status, payment_method, payment_url, created_at, approved_at'
    )
  ...
  const summary = (data ?? []).reduce(
    ...
    { received: 0, pending: 0, received_count: 0, pending_count: 0 }
  );
