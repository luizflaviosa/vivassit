...
interface ProfissionaisViewProps {
  initialDoctors: Doctor[] | null;
}

function ProfissionaisInner({ initialDoctors }: ProfissionaisViewProps) {
  const me = useMe();
  ...
  const [loading, setLoading] = useState<boolean>(initialDoctors === null);
  ...
  useEffect(() => {
    // Pula refetch se Server ja entregou lista.
...
export default function ProfissionaisView(props: ProfissionaisViewProps) {
  return <ProfissionaisInner {...props} />;
}
