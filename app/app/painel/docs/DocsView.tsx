...
interface DocsViewProps {
  initialDocs: DocWithPatient[] | null;
}

function DocsInner({ initialDocs }: DocsViewProps) {
  const me = useMe();
  const [docs, setDocs] = useState<DocWithPatient[]>(initialDocs ?? []);
  const [loading, setLoading] = useState<boolean>(initialDocs === null);
  const [statusFilter, setStatusFilter] = useState<DocStatus | ''>('');
  // Server entregou docs sem filtro de status (statusFilter='' inicial).
  // Pula primeiro mount; mudanca de filtro dispara refetch.
  const firstMount = useRef(initialDocs !== null);
  ...
  if (!me?.tenant_id && initialDocs === null) return null;
