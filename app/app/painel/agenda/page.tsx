...
function AgendaInner() {
  ...
  const fetchEvents = async (silent = false) => {
    ...
    try {
      const res = await fetch('/api/painel/agenda/events?back=2&forward=60', { cache: 'no-store' });
      const json = await res.json();
      if (json.success) {
        setEvents(json.events ?? []);
        setRequiresReauth(false);
        setErrMsg(null);
      } else if (json.requires_reauth) {
        setRequiresReauth(true);
        setErrMsg(json.message ?? 'Reautentique com Google.');
      } else {
        setErrMsg(json.message ?? 'Não consegui carregar a agenda.');
      }
    } catch {
  ...
  const handleReauth = async () => {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${origin}/auth/callback?next=/painel/agenda`,
    ...
    <div className="space-y-6">
      ...
      {requiresReauth && (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          className="rounded-2xl border border-amber-200 bg-amber-50/50 p-6 sm:p-8 text-center"
        >
          ...
          <h2 className="text-[18px] font-semibold text-zinc-900 mb-2">
            Permissão da agenda Google necessária
          </h2>
          <p className="text-[14px] text-zinc-600 max-w-md mx-auto leading-relaxed mb-5">
            {errMsg ?? 'Reautentique com Google pra liberar leitura da sua agenda.'}
          </p>
          <button
            type="button"
      ...
      {!loading && !requiresReauth && events.length === 0 && (
      ...
      {!loading && !requiresReauth && events.length > 0 && view === 'list' && (
        <div className="space-y-6">
      ...
      {!loading && !requiresReauth && events.length > 0 && view === 'week' && (
        <div>
