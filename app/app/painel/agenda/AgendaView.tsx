...
interface AgendaViewProps {
  initialDoctors: DoctorOption[] | null;
  initialActiveDoctor: string | null;
  initialEvents: AgendaEvent[] | null;
  initialSetupIssue: SetupIssue | null;
}
...
function AgendaInner({
  ...
  const [loading, setLoading] = useState<boolean>(initialEvents === null && initialSetupIssue === null);
  const [refreshing, setRefreshing] = useState(false);
  const [setupIssue, setSetupIssue] = useState<SetupIssue | null>(initialSetupIssue);
  const [view, setView] = useState<View>('week');
  const [date, setDate] = useState<Date>(new Date());
  const [selected, setSelected] = useState<AgendaEvent | null>(null);
  const [doctors, setDoctors] = useState<DoctorOption[]>(initialDoctors ?? []);
  const [activeDoctor, setActiveDoctor] = useState<string | null>(initialActiveDoctor);
  // Marca o primeiro mount pra pular o useEffect inicial de events
  // (dados ja vieram do Server). Trocas subsequentes de doctor disparam refetch normal.
  const eventsFirstMount = useRef(true);
  ...
  // Carrega lista de profissionais (skip se Server ja entregou)
  useEffect(() => {
    if (initialDoctors !== null) return;
  ...
  useEffect(() => {
    // Pula primeiro mount se Server ja entregou events (skipa refetch redundante).
    // Trocas subsequentes de activeDoctor disparam refetch normal.
    if (eventsFirstMount.current && (initialEvents !== null || initialSetupIssue !== null)) {
      eventsFirstMount.current = false;
      return;
    }
    eventsFirstMount.current = false;
    fetchEvents();
