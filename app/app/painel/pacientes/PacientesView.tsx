...
function PacientesInner({ initialPatients }: PacientesViewProps) {
  const me = useMe();
  const tenantId = me?.tenant_id ?? '';

  // First paint instantaneo se Server entregou lista.
  const [patients, setPatients] = useState<Patient[]>(initialPatients ?? []);
  const [loading, setLoading] = useState<boolean>(initialPatients === null);
  ...
  useEffect(() => {
    // Pula refetch se Server ja entregou lista (initialPatients !== null).
    if (initialPatients !== null) return;
    if (!tenantId) {
      setLoading(false);
      return;
    }
