...
interface MensagensViewProps {
  ...
  initialSummary: { total: number; unique_sessions: number } | null;
}

function MensagensInner({ initialMessages, initialSummary }: MensagensViewProps) {
  const me = useMe();
  const tenantId = me?.tenant_id ?? '';
  const [messages, setMessages] = useState<MessageRow[]>(initialMessages ?? []);
  const [summary, setSummary] = useState<{ total: number; unique_sessions: number } | null>(initialSummary);
  const [loading, setLoading] = useState<boolean>(initialMessages === null);
  const [selectedSession, setSelectedSession] = useState<string | null>(null);
  // Pula primeiro mount se Server entregou (selectedSession=null inicial bate
  // com o que o Server buscou: lista geral). Trocas de sessao disparam refetch.
  const firstMount = useRef(initialMessages !== null);
  ...
  if (!tenantId && initialMessages === null) return null;
...
export default function MensagensView(props: MensagensViewProps) {
  return <MensagensInner {...props} />;
}
