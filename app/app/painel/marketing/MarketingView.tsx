...
interface MarketingViewProps {
  ...
  initialPosts: Post[];
  initialEligibleReviews: number;
  initialRegionDemand: RegionDemand | null;
  initialMarketTrends: MarketTrends | null;
  initialGbpInsights: GbpInsights | null;
  initialCompetitors: Competitors | null;
...
function MarketingInner({
  ...
  const [scoreData, setScoreData] = useState<ScoreData | null>(initialScoreData);
  const [recs, setRecs] = useState<Recommendation[]>(initialRecs);
  const [posts, setPosts] = useState<Post[]>(initialPosts);
  const [eligibleReviews, setEligibleReviews] = useState(initialEligibleReviews);
  const [regionDemand, setRegionDemand] = useState<RegionDemand | null>(initialRegionDemand);
  const [regionRefreshing, setRegionRefreshing] = useState(false);
  const [marketTrends, setMarketTrends] = useState<MarketTrends | null>(initialMarketTrends);
  const [gbpInsights, setGbpInsights] = useState<GbpInsights | null>(initialGbpInsights);
  const [gbpRefreshing, setGbpRefreshing] = useState(false);
  const [competitors, setCompetitors] = useState<Competitors | null>(initialCompetitors);
  const [competitorsRefreshing, setCompetitorsRefreshing] = useState(false);
  const [loading, setLoading] = useState<boolean>(!initialLoaded);
  // Marca o primeiro mount pra pular o useEffect inicial de fetchData
  // (Server ja entregou tudo). approve(), refresh*() etc invocam fetchData
  // normalmente depois.
  const fetchDataFirstMount = useRef(initialLoaded);
...
export default function MarketingView(props: MarketingViewProps) {
